import time
from board_app.models import User, Board, BoardMembership, DrawingAction, ActiveConnection, PermissionEnum
...
class TestMultipleUsersInGroup:
    ...
    def test_multiple_users_in_same_board(self, app, test_board_with_members, test_user, test_user2):
    ...
    def test_active_connections_tracking(self, app, test_board_with_members, test_user, test_user2):
        ...
        with app.app_context():
            ...
            connections = ActiveConnection.query.filter_by(board_id=test_board_with_members).all()
    ...
    def test_get_active_users(self, app, test_board_with_members, test_user, test_user2):
...
class TestPersistenceOfActions:
    ...
    def test_multiple_drawing_actions_persistence(self, app, test_board, test_user):
        ...
        with app.app_context():
            ...
            actions = DrawingAction.query.filter_by(board_id=test_board).order_by(DrawingAction.created_at).all()
    ...
    def test_clear_board_action(self, app, test_board, test_user):
        ...
        with app.app_context():
            ...
            actions_before = DrawingAction.query.filter_by(board_id=test_board).count()
            ...
            actions_after = DrawingAction.query.filter_by(board_id=test_board).count()
...
class TestPresenceUpdates:
    ...
    def test_get_active_users_functionality(self, app, test_board_with_members, test_user, test_user2):
        """Test that get_active_users returns correct users."""
        with app.app_context():
            # Add active connections for both users
            add_active_connection(test_board_with_members, test_user, 'channel1')
...
class TestIntegrationScenarios:
    ...
    def test_complete_user_workflow(self, app, test_board, test_user):
        ...
        with app.app_context():
            ...
            action = DrawingAction.query.filter_by(action_id='workflow-action-1').first()
            assert action is not None
    ...
    def test_multiple_users_collaboration(self, app, test_board_with_members, test_user, test_user2):
        ...
        with app.app_context():
            ...
            result1 = save_drawing_action(test_board_with_members, test_user, drawing_data1)
            ...
            result2 = save_drawing_action(test_board_with_members, test_user2, drawing_data2)
            assert result2 is not None
            
            # Check that both actions were saved
            actions = DrawingAction.query.filter_by(board_id=test_board_with_members).all()
